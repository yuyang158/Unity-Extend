...
const im = require('./im-notify-wechat')

const sqlConnection = mysql.createConnection(config.get('log_database'))
...
function queryMethod(pageIndex, typ, cb) {
  ...
  sqlConnection.query(
    "SELECT * FROM `" + typ + "` ORDER BY `id` LIMIT 50 OFFSET ?",
    [offset],
    (err, results) => {
      if (err) {
        console.error(err)
