...
namespace Extend.Editor.Preview {
	public abstract class PreviewBase : ObjectPreview {
		private class Styles {
			...
			public readonly GUIStyle preLabel = "PrefixLabel";
		}
		...
		public override void OnPreviewSettings() {
			...
			EditorGUI.BeginChangeCheck();
			var speed = PreviewSlider(PlaybackSpeed, 0.03f);
			if( EditorGUI.EndChangeCheck() ) {
				PlaybackSpeed = speed;
			}
		...
		protected abstract bool HasStaticPreview();
		...
		/// 模拟-开启
		/// </summary>
		protected virtual void SimulateEnable() {
			SimulateDisable();
			SetSimulateMode();
		...
		/// 模拟-停止
		/// </summary>
		protected virtual void SimulateDisable() {
			EditorApplication.update -= InspectorUpdate;
			m_RunningTime = 0f;
			m_Playing = false;
		}
		...
		/// </summary>
		protected virtual void SetSimulateMode() {
			SimulateDisable();
		}
		...
		private static float PreviewSlider(float val, float snapThreshold) {
