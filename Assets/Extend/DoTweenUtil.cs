...
namespace Extend {
	...
	public static class DoTweenUtil {
		public static Tweener DoTypewriter(TMP_Text text, string content, float duration) {
			var tweener = DOTween.To(() => string.Empty, x => text.text = x, content, duration);
		...
		public static void CompleteTweener(Tweener tweener) {
			tweener.Kill(true);
		}
		...
		public static Tweener DoGraphicFade(Graphic t, float end, float duration, float delay = 0,
			bool autoKill = false) {
			return t.DOFade(end, duration).SetAutoKill(autoKill).SetDelay(delay);
		}

		public static Tweener DoSpriteRenderFade(SpriteRenderer t, float end, float duration, float delay = 0,
			bool autoKill = false) {
			return t.DOFade(end, duration).SetAutoKill(autoKill).SetDelay(delay);
		}
		...
		public static void DoBezierPath(Transform t, Vector3 startPoint, System.Action onEnd,
			...
			t.LookAt(path[1]);
			t.DOPath(path, duration, PathType.CubicBezier).OnComplete(() => onEnd?.Invoke()).SetDelay(delay)
				.SetLookAt(0.01f);
		}
		...
		public static void DoLocalJump(Transform transform, float endValueX, float endValueY, float endValueZ,
			...
			transform.DOLocalJump(new Vector3(endValueX, endValueY, endValueZ), jumpPower, numJumps, duration,
				snapping);
		}
		...
		public static void DoCameraFovPunch(CinemachineVirtualCamera c, float fov, float delay, float duration,
			int vibrato, float elasticity, System.Action onEnd = null) {
			DOTween.Punch(() => new Vector3(fov, 0, 0), x => { c.m_Lens.FieldOfView = x.x; }, new Vector3(1, 0, 0),
				duration, vibrato, elasticity).SetDelay(delay).OnComplete(() => onEnd?.Invoke());
		}

		public static void DoRewindThenRecreateTweenAndPlay(GameObject go) {
			var animations = go.GetComponents<DOTweenAnimation>();
