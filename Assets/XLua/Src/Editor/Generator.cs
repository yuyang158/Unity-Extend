    ...
    public static class Generator
    {
        ...
        static void getInterfaceInfo(Type type, LuaTable parameters)
        {
            ...
            parameters.Set("methods", itfs.SelectMany(i => i.GetMethods())
                .Where(method => !method.IsSpecialName && !method.IsGenericMethod && !method.Name.StartsWith("op_") && !method.Name.StartsWith("add_") && !method.Name.StartsWith("remove_") && !isMemberInBlackList(method)) //GenericMethod can not be invoke becuase not static info available!
                    .ToList());
