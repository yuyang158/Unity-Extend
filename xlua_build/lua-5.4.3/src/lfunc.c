...
** at the given level.
*/
UpVal *moonF_findupval (lua_State *L, StkId level) {
...
static void prepcallclosemth (lua_State *L, StkId level, int status, int yy) {
  ...
  if (status == CLOSEKTOP)
    ...
    moonD_seterrorobj(L, status, level + 1);  /* set error object */
  }
...
*/
void moonF_newtbcupval (lua_State *L, StkId level) {
...
void moonF_unlinkupval (UpVal *uv) {
...
*/
void moonF_closeupval (lua_State *L, StkId level) {
  UpVal *uv;
  StkId upl;  /* stack index pointed by 'uv' */
  while ((uv = L->openupval) != NULL && (upl = uplevel(uv)) >= level) {
    TValue *slot = &uv->u.value;  /* new position for value */
    lua_assert(uplevel(uv) < L->top);
    moonF_unlinkupval(uv);  /* remove upvalue from 'openupval' list */
...
** level.
*/
void moonF_close (lua_State *L, StkId level, int status, int yy) {
...
void moonF_freeproto (lua_State *L, Proto *f) {
