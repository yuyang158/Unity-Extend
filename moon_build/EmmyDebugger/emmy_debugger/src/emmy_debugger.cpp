...
void Debugger::Attach()
{
	...
	if (!manager->helperCode.empty())
	{
		...
		{
			const int t = moon_gettop(L);
			// 判断是不是主lua_State
			int ret = moon_pushthread(L);
			if (ret == 1)
			{
				const int r = moonL_loadstring(L, manager->helperCode.c_str());
				if (r == LUA_OK)
				{
					if (moon_pcall(L, 0, 0, 0) != LUA_OK)
					{
...
bool Debugger::GetStacks(std::vector<std::shared_ptr<Stack>>& stacks, StackAllocatorCB alloc)
{
	...
	{
		lua_Debug ar{};
		if (!moon_getstack(L, level, &ar))
		{
			break;
		}
		if (!moon_getinfo(L, "nSlu", &ar))
		{
			continue;
		}
		auto stack = alloc();
		...
		// get variables
		{
			for (int i = 1;; i++)
			{
				const char* name = moon_getlocal(L, &ar, i);
				if (name == nullptr)
				{
					break;
				}
			...
			if (moon_getinfo(L, "f", &ar))
			{
				const int fIdx = moon_gettop(L);
				for (int i = 1;; i++)
				{
					const char* name = moon_getupvalue(L, fIdx, i);
					if (!name)
					{
						break;
					}
...
{
	if (moon_getmetatable(L, valueIndex))
	{
		const int metaIndex = moon_gettop(L);
		if (!lua_isnil(L, metaIndex))
		{
			moon_pushstring(L, method);
			moon_rawget(L, metaIndex);
			if (lua_isnil(L, -1))
			{
				// The meta-method doesn't exist.
				lua_pop(L, 1);
				moon_remove(L, metaIndex);
				return false;
			}
			moon_pushvalue(L, valueIndex);
			result = moon_pcall(L, 1, numResults, 0);
		}
		moon_remove(L, metaIndex);
		return true;
	}
...
{
	const void* pointer = moon_topointer(L, index);
	std::stringstream ss;
	ss << moon_typename(L, moon_type(L, index)) << "(0x" << std::hex << pointer << ")";
	return ss.str();
}
...
void Debugger::GetVariable(std::shared_ptr<Variable> variable, int index, int depth, bool queryHelper)
{
	...
	const int topIndex = moon_gettop(L);
	index = moon_absindex(L, index);
	CacheValue(index, variable);
	const int type = moon_type(L, index);
	const char* typeName = moon_typename(L, type);
	...
	case LUA_TBOOLEAN:
		{
			const bool v = moon_toboolean(L, index);
	...
	case LUA_TTABLE:
		{
			...
			const void* tableAddr = moon_topointer(L, index);
			moon_pushnil(L);
			while (moon_next(L, index))
			{
				// k: -2, v: -1
				if (depth > 1)
				{
					...
					const auto t = moon_type(L, -2);
					v->nameType = t;
					if (t == LUA_TSTRING || t == LUA_TNUMBER || t == LUA_TBOOLEAN)
					{
						moon_pushvalue(L, -2); // avoid error: "invalid key to 'next'" ???
...
void Debugger::CacheValue(int valueIndex, std::shared_ptr<Variable> variable) const
{
	...
	const int type = moon_type(L, valueIndex);
	if (type == LUA_TUSERDATA || type == LUA_TTABLE)
	{
		const int id = cacheId++;
		variable->cacheId = id;
		const int top = moon_gettop(L);
		moon_getfield(L, LUA_REGISTRYINDEX, CACHE_TABLE_NAME); // 1: cacheTable|nil
		if (lua_isnil(L, -1))
		{
			lua_pop(L, 1); //
			lua_newtable(L); // 1: {}
			moon_setfield(L, LUA_REGISTRYINDEX, CACHE_TABLE_NAME); //
			moon_getfield(L, LUA_REGISTRYINDEX, CACHE_TABLE_NAME); // 1: cacheTable
		}

		moon_pushvalue(L, valueIndex); // 1: cacheTable, 2: value

		// snprintf 性能不够，问题也很大，这里采用c++标准算法
		std::string key = std::to_string(id);
		moon_setfield(L, -2, key.c_str()); // 1: cacheTable
...
void Debugger::ClearCache() const
{
	...
	moon_getfield(L, LUA_REGISTRYINDEX, CACHE_TABLE_NAME);
	if (!lua_isnil(L, -1))
	{
		moon_pushnil(L);
		moon_setfield(L, LUA_REGISTRYINDEX, CACHE_TABLE_NAME);
...
{
	if (mask == 0)
		moon_sethook(L, nullptr, mask, 0);
	else
		moon_sethook(L, EmmyFacade::HookLua, mask, 0);
}
...
int FixPath(lua_State* L)
{
	...
	moon_getglobal(L, "emmy");
	if (lua_istable(L, -1))
	{
		moon_getfield(L, -1, "fixPath");
...
std::string Debugger::GetFile(lua_Debug* ar) const
{
	...
	if (strlen(file) > 0 && file[0] == '@')
		file++;
	moon_pushcclosure(L, FixPath, 0);
	moon_pushstring(L, file);
	const int result = moon_pcall(L, 1, 1, 0);
	if (result == LUA_OK)
	{
...
int EnvIndexFunction(lua_State* L)
{
	...
	lua_pop(L, 1);
	// _ENV
	moon_getfield(L, upvalues, "_ENV");
	if (lua_istable(L, -1))
	{
		moon_getfield(L, -1, name); // _ENV[name]
		if (lua_isnil(L, -1) == 0)
		{
			return 1;
		}
...
bool Debugger::CreateEnv(int stackLevel)
{
	...
	lua_Debug ar{};
	if (!moon_getstack(L, stackLevel, &ar))
	{
		return false;
	}
	if (!moon_getinfo(L, "nSlu", &ar))
	...
	lua_newtable(L);
	const int env = moon_gettop(L);
	lua_newtable(L);
	const int envMetatable = moon_gettop(L);
	lua_newtable(L);
	const int locals = moon_gettop(L);
	lua_newtable(L);
	const int upvalues = moon_gettop(L);

	int idx = 1;
	// local values
	while (true)
	{
		const char* name = moon_getlocal(L, &ar, idx++);
		if (name == nullptr)
			break;
		if (name[0] == '(')
		{
	...
	if (moon_getinfo(L, "f", &ar))
	{
		...
		while (true)
		{
			const char* name = moon_getupvalue(L, fIdx, idx++);
			if (name == nullptr)
				break;
			moon_setfield(L, upvalues, name);
		}
		lua_pop(L, 1);
	}
	...
	moon_pushcclosure(L, EnvIndexFunction, 2);

	// envMetatable.__index = EnvIndexFunction
	moon_setfield(L, envMetatable, "__index");
	// setmetatable(env, envMetatable)
	moon_setmetatable(L, env);
...
void Debugger::CheckDoString()
{
	...
	if (!doStringList.empty())
	{
		...
		{
			const int r = moonL_loadstring(L, code.c_str());
			if (r == LUA_OK)
			{
				moon_pcall(L, 0, 0, 0);
			}
...
bool Debugger::DoEval(std::shared_ptr<EvalContext> evalContext)
{
	...
	if (evalContext->cacheId > 0)
	{
		...
		{
			moon_getfield(L, -1, std::to_string(evalContext->cacheId).c_str()); // 1: cacheTable, 2: value
	...
	int r = moonL_loadstring(L, statement.c_str());
	if (r == LUA_ERRSYNTAX)
	{
    ...
    moon_setfenv(L, fIdx);
#else //52 & 53
    moon_setupvalue(L, fIdx, 1);
#endif
	// call function() return expr end
	r = moon_pcall(L, 0, 1, 0);
	if (r == LUA_OK)
	{
